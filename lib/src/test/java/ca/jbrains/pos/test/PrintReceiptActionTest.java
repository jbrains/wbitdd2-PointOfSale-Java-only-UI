...
public class PrintReceiptActionTest {
    ...
    void completedPurchase() {
        ...
        FormatReceipt formatReceipt = new FormatReceipt(new FormatItem(), formatTotal) {
            @Override
    ...
    void noCompletedPurchaseAndNoPurchaseInProgress() {
        assertEquals("There is no completed purchase, therefore I can't print a receipt",
                new StandardPrintReceiptAction(
                        new NoHistoryPurchaseAccumulator(), new FormatReceipt(new FormatItem(), null)).printReceipt());
    }
    ...
    static class FormatReceiptTest {
        ...
        void noItems() {
            ...
            assertEquals("Total: CAD 0.00",
                    new FormatReceipt(new FormatItem(), formatTotal).formatReceipt(new Purchase(0, List.of())));
        }

        @Test
        void oneItem() {
            final FormatTotal formatTotal = new FormatTotal(new FormatMonetaryAmount(Locale.ENGLISH));
            final FormatReceipt formatReceipt = new FormatReceipt(new FormatItem(), formatTotal);
        ...
        static class FormatItemTest {
            ...
            void noSpacesBetweenBarcodeAndPrice() {
                final FormatReceipt formatReceipt = new FormatReceipt(new FormatItem(), null);
