...
public class TestSellOneItem {
    ...
    @Test
    void priceNotFound() {
        String response = new PointOfSale.HandleBarcode(priceNotFoundCatalog, null).handleBarcode(new Barcode("99999")
        );
    ...
    void givenBarcodeIs1111ShouldDisplayProductNotFoundMessage() {
        String response = new PointOfSale.HandleBarcode(priceNotFoundCatalog, null).handleBarcode(Barcode.makeBarcode("1111").get()
        );
    ...
    @Test
    void priceFound() {
        String response = new PointOfSale.HandleBarcode(priceFoundCatalog, new PurchaseAccumulator() {
            @Override
        ...
        }).handleBarcode(Barcode.makeBarcode("99999").get()
        );
    ...
    void rememberTheScannedItemWhenProductIsFound() {
        ...
        new PointOfSale.HandleBarcode(priceFoundCatalog, purchaseProvider).handleBarcode(Barcode.makeBarcode("::any barcode::").get()
        );
